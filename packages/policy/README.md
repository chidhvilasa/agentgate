...
[AgentGate](https://github.com/chidhvilasa/agentgate)'s gateway. Depends only on `@chidhvilasa/protocol` and a
...
> the full policy language. Installing `@chidhvilasa/policy` directly is normally only useful if you are building
> your own tooling that needs to load or evaluate an AgentGate policy file outside the gateway itself.

```sh
npm install @chidhvilasa/policy
```

```ts
import { loadPolicyFile, evaluate } from '@chidhvilasa/policy';
```
