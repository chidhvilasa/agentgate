...
| Installing the gateway tarball alone genuinely fails (a real limitation, not hypothetical) | Reproduced manually with `npm install <gateway-tarball-only>` → real `npm error 404` for `@chidhvilasa/policy`; documented in `docs/DEVELOPMENT.md#installability`, not silently omitted |
...
| Every publishable package (`@chidhvilasa/protocol`, `@chidhvilasa/policy`, `@chidhvilasa/gateway`) is not `private`, carries accurate `repository`/`homepage`/`bugs`/`keywords`/`publishConfig.access:"public"`, and a restrictive `files` allowlist | `scripts/check-release-consistency.mjs` (also unit-tested in `scripts/check-release-consistency.test.mjs`, 7 cases) |
...
| npm registry state for all three package names, and whether `@agentgate` scope ownership can be verified in this environment | `npm whoami` → `ENEEDAUTH`; `npm view @chidhvilasa/{protocol,policy,gateway}` → all 404; scope ownership genuinely unverifiable without an authenticated npm session (not "confirmed available") |
